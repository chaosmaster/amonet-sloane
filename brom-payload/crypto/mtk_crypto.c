...
int xor_data(uint8_t *a, uint8_t *b, uint8_t *out_buf, int len) {
    int i;
    for (i = 0; i < len; i++) {
        out_buf[i] = a[i] ^ b[i];
    }
...
int mtk_crypto_hmac_sha256_by_devkey_using_seed(uint8_t* seed, uint8_t* data, uint32_t data_len, uint8_t* mac) {
...
int mtk_crypto_hmac_sha256_by_devkey(uint8_t* data, uint32_t data_len, uint8_t* mac) {
    uint8_t seed[16] = { 0 };
    uint32_t dev_val;
    ...
    xor_data(seed, &dev_val, seed, 4);
    dev_val = get_devinfo_with_index(13);
    xor_data(seed + 4, &dev_val, seed + 4, 4);
